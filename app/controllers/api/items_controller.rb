class Api::ItemsController < Api::ApiController
  ...
  def sync
    ...
    last_updated = DateTime.now + 1/1000
    saved_items = _sync_save()
    if saved_items.length > 0
      last_updated = saved_items.sort_by{|m| m.created_at}.first.updated_at + 1/1000
    end
    render :json => {:retrieved_items => retrieved_items, :saved_items => saved_items, :last_updated => last_updated}
  end
  ...
  def _sync_get
    if params[:updated_after]
      items = @user.items.where("updated_at > ?", params[:updated_after].to_time).all
    else
